...
#design for Llama, pretrained in english also supported 
class Llama_Embedding(nn.Module):
    def __init__(self, config: Dict, max_len: int=None):
        super(Llama_Embedding,self).__init__()
        if config["text_embedding"]["add_new_token"]:
            self.tokenizer = Text_tokenizer(config)
            self.embedding = AutoModelForCausalLM.from_pretrained(config["text_embedding"]["text_encoder"])
