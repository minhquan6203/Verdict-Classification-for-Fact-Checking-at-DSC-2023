...
class CoAttentionEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pos_embedding = SinusoidPositionalEmbedding(config["multi_encoder"]['d_model'])
        self.vision_layer_norm = nn.LayerNorm(config["multi_encoder"]['d_model'])
        self.language_layer_norm = nn.LayerNorm(config["multi_encoder"]['d_model'])

        self.d_model = config["multi_encoder"]['d_model']

        # cross-attention layers
        self.vision_language_attn_layers = nn.ModuleList([EncoderLayer(config) for _ in range(config["multi_encoder"]['layers'])])
        self.language_vision_attn_layers = nn.ModuleList([EncoderLayer(config) for _ in range(config["multi_encoder"]['layers'])])

        # self-attention layers
        self.vision_self_attn_layers = nn.ModuleList([EncoderLayer(config) for _ in range(config["multi_encoder"]['layers'])])
        self.language_self_attn_layers = nn.ModuleList([EncoderLayer(config) for _ in range(config["multi_encoder"]['layers'])])
...
class CrossModalityEncoderLayer(nn.Module):
    def __init__(self, config):
        ...
        self.vision_language_mhattn = MultiHeadAtt(config)
        self.language_vision_mhattn = MultiHeadAtt(config)

        # self-attention modules
        self.vision_mhattn = MultiHeadAtt(config)
        self.language_mhattn = MultiHeadAtt(config)

        # pff
        self.vision_pff = PositionWiseFeedForward(config)
        self.language_pff = PositionWiseFeedForward(config)
...
class CrossModalityEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.d_model = config["multi_encoder"]['d_model']
        self.layers = nn.ModuleList([CrossModalityEncoderLayer(config) for _ in range(config["multi_encoder"]['layers'])])
...
class GuidedAttentionEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pos_embedding = SinusoidPositionalEmbedding(config["multi_encoder"]['d_model'])
        self.layer_norm = nn.LayerNorm(config["multi_encoder"]['d_model'])
        self.d_model = config["multi_encoder"]['d_model']

        self.guided_attn_layers = nn.ModuleList([GuidedEncoderLayer(config) for _ in range(config["multi_encoder"]['layers'])])
        self.language_self_attn_layers = nn.ModuleList([GuidedEncoderLayer(config) for _ in range(config["multi_encoder"]['layers'])])
